#!/usr/bin/env python3
"""Spotify Clone - Automated Discovery Engine

STRICT AI CALL LIMITING: Maximum 1 AI API call per run
Optimized prompts to prevent token overflow
Star Rating Integration: Excludes 1-2 star rated songs
Configurable AI Backend: OpenAI library or Native Gemini SDK with caching

Improvements:
- Error handling with proper return types (None vs empty collections)
- Daily cache invalidation with date tracking
- Exponential backoff retry logic for API calls
- Config validation on startup
- Complete type hints throughout
- Duplicate auth code extracted to utility
- Configurable hard-coded values
- Dry-run mode support
- Async operations for parallel processing
- SQLite database for ratings cache
- Recommendation deduplication
- Refactored long methods into smaller functions
- No global state (moved into classes)
"""
...
LOG_FILE = SCRIPT_DIR / "discovery_engine.log"
LOCK_FILE = SCRIPT_DIR / "discovery_engine.lock"
RATINGS_DB = SCRIPT_DIR / "ratings_cache.db"
...
def subsonic_auth_params(username: str, password: str) -> Dict[str, str]:
    ...
    Note: Uses MD5 as required by Subsonic API spec (not for security).
    """
...
class NavidromeAPI:
    ...
    def test_connection(self) -> bool:
        ...
        if response:
            logger.info("Navidrome connected successfully")
            return True
...
class AIRecommendationEngine:
    """AI music recommendations with configurable backend and state management."""
    ...
    def __init__(
        ...
        self.cache_file = SCRIPT_DIR / "gemini_cache.json"
        ...
        if self.backend == "gemini":
            if not GEMINI_SDK_AVAILABLE:
                ...
                logger.error("Run: pip install google-genai --break-system-packages")
                sys.exit(1)
            ...
            logger.info("OpenAI library initialized")
    ...
    def _build_cached_context(
        ...
        negative_context = ""
        if low_rated_songs:
            # Don't log full song titles (privacy)
            negative_sample = [
                f"{s.get('artist','')} - {s.get('title','')}"
...
Use songs from this library for "library songs" and recommend NEW similar songs."""
    ...
    def _generate_with_gemini(
        ...
        # Set thinking budget (recommended: 2000-5000 tokens for structured tasks)
        thinking_budget = 5000  # Adjust as needed
...
class SpotifyCloneEngine:
    ...
    def run(self) -> None:
        ...
        logger.info("=" * 70)
        logger.info("SPOTIFY CLONE - Starting: %s", start_time.strftime("%Y-%m-%d %H:%M:%S"))
        logger.info("=" * 70)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Spotify Clone Discovery Engine")
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    lock = acquire_lock()
    try:
        engine = SpotifyCloneEngine(CONFIG_FILE, dry_run=args.dry_run)
        engine.run()
